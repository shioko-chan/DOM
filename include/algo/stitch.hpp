...
public:
  ...
  explicit DSMStitcher(const fs::path& temporary_save_path) noexcept :
      temporary_save_path(temporary_save_path) {
    check_or_create_path(temporary_save_path);
  }

  auto stitch(ImgsData& imgs_data, const DSM& dsm, Progress& progress) noexcept -> cv::Mat {
    ...
    for(auto& img_data : imgs_data) {
      THIS_MESSAGE("processing: {}", img_data.name());
...
private:
  ...
  void project_image(const ImgData& img_data, const DSM& dsm, cv::Mat& result, cv::Mat& weights) const noexcept {
