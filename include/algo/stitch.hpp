...
public:
  DSMStitcher() = delete;
  explicit DSMStitcher(const fs::path& temporary_save_path, double resolution = 0.5,
                      double world_min_x = 0, double world_min_y = 0) :
      temporary_save_path(temporary_save_path), 
      resolution(resolution),
      world_min_x_(world_min_x),
      world_min_y_(world_min_y) {
    check_or_create_path(temporary_save_path);
  }

  auto stitch(const ImgsData& imgs_data, const cv::Mat& dsm, Progress& progress) -> cv::Mat {
    ...
    for(const auto& img_data : imgs_data) {
      THIS_MESSAGE("processing image: {}", img_data.name());
      processImage(img_data, dsm, result, weights, resultMask);
      progress.update();
    }
...
private:
  ...
  cv::Point2f backProject(const cv::Mat& dsm, int row, int col, const ImgData& img_data) const {
    ...
    cv::Mat img_point = img_data.M() * camera_point;
    float u = static_cast<float>(img_point.at<double>(0, 0) / img_point.at<double>(2, 0));
    float v = static_cast<float>(img_point.at<double>(1, 0) / img_point.at<double>(2, 0));
    
    return cv::Point2f(u, v);
  }
  
  bool isPointInImage(const cv::Point2f& point, const cv::Size& img_size) const {
    return point.x >= 0 && point.y >= 0 && 
           point.x < img_size.width && point.y < img_size.height;
  }
  
  void processImage(const ImgData& img_data, const cv::Mat& dsm, cv::Mat& result, 
                    cv::Mat& weights, cv::Mat& resultMask) const {
    ...
    for (int row = 0; row < dsm.rows; ++row) {
      for (int col = 0; col < dsm.cols; ++col) {
        ...
        cv::Point2f img_point = backProject(dsm, row, col, img_data);
        ...
        if (isPointInImage(img_point, img_size)) {
          ...
          if (p1.x < img_size.width && p2.y < img_size.height && 
              p3.x < img_size.width && p3.y < img_size.height) {
          ...
          cv::Vec3f& result_pixel = result.at<cv::Vec3f>(row, col);
          float& pixel_weight = weights.at<float>(row, col);
    ...
    for (int row = 0; row < result.rows; ++row) {
      for (int col = 0; col < result.cols; ++col) {
        ...
        if (weight > 0) {
          ...
          normalized_result.at<cv::Vec3b>(row, col) = cv::Vec3b(
              static_cast<uchar>(pixel[0]),
