...
inline void filter_too_few_points(TriResVec* tri_res_vec, int min_points = 2) {
  std::erase_if(*tri_res_vec, [min_points](const auto& tri_res) noexcept {
    return tri_res.pnt2d_idx_vec.size() < min_points;
  });
}

inline void filter_near_observes(ImgsData& imgs_data, TriResVec* tri_res_vec, double threshold = 5.0) {
