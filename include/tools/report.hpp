#ifndef SKYMERGE_terminate_with_error_HPP
#define SKYMERGE_terminate_with_error_HPP
...
inline auto format_exception(const std::filesystem::filesystem_error& exception) noexcept -> std::string {
  return std::format(
...
inline auto format_ort_error_code(OrtErrorCode code) noexcept -> std::string_view {
...
inline auto format_exception(const Ort::Exception& exception) noexcept -> std::string {
  return std::format(
      ...
      "  Error Code   : {}\n"
      "  Message      : {}\n",
      format_ort_error_code(exception.GetOrtErrorCode()),
      exception.what());
}
...
[[noreturn]] void
terminate_with_error(const Exception& exception, std::string_view error_msg, const Args&... args) noexcept {
  THIS_LOG_ERROR("{}\nDetail: {}", std::vformat(error_msg, std::make_format_args(args...)), format_exception(exception));
  std::terminate();
}

template <typename... Args>
[[noreturn]] void terminate_with_error(std::string_view error_msg, const Args&... args) noexcept {
